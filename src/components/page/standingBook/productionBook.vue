    ...
    <div class="main-top">
      <div class="breadcrumb">
        <a href="#">
          ...
          <span class="text-primary-font">企业监管</span>
        </a>
        <span class="sep">&gt;</span>
        <a href="#">
          <i class="icon icon-zhuanyun"></i>
          <span class="text-primary">台账监控</span>
        </a>
      </div>
    </div>
    <div class="panel">
            ...
            <li class="filter-item">
              ...
              <div class="filter-con">
                <!--<el-cascader-->
                  ...
                  <!--clearable-->
                <!--&gt;</el-cascader>-->
                <el-select v-model="detailName"  clearable  placeholder="请选择">
                  <el-option
                          v-for="item in entNameList"
                          :key="item.code"
                          :label="item.name"
                          :value="item.code">
                  </el-option>
      ...
      <div class="main-container">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane v-if="roleType == 5" label="生产台账" name="first">
          ...
          <el-tab-pane
            v-if="roleType == 5 || roleType == 1 || roleType == 7"
            label="贮存台账"
            name="second"
          >
            <table class="table-default" v-if="true">
              ...
              <tbody>
                <tr v-for="(item, i) in tableDatas2" :key="i">
                  <td style="text-align: center; width: 6%">
                ...
                <table class="table-text">
                  <tr>
                    ...
                    <td>
                      ...
                      <div class="list-text">{{tableData.type == 3 ? '--':tableData.containerName }}</div>
                    </td>
                    <td>
                      ...
                      <div v-if="activeName == 'third'" class="list-text">--</div>
                      <div v-if="activeName == 'four'" class="list-text">--</div>
                    </td>
                  </tr>
                  <tr>
                    <td>
...
import updateProBook from "./updateProBook";
import qs from "qs";
export default {
  name: "RegisterHome",
  components: { updateProBook },
  data() {
    return {
      typeNameList: [],
      typeId: "",
  ...
  methods: {
    ...
    getEntNameList() {
      var _this = this;
      _this.$axios
              .get("/api/regulatory/type/getEntTypeNameLists")
              .then((res) => {
                if (res.code == 0 && res.data != null) {
                  _this.entNameList = res.data;
                }
              })
              .catch(function (err) {
                console.log(err);
    ...
    getPrinting(item) {
      ...
      params.append("newDate", item.storageTime.split(" ")[0]);
    ...
    exportExcel() {
      ...
      params.append("detailName", _this.detailName);
      _this.$axios
    ...
    selectentErpriseTSBook() {
      ...
      _this.$axios
        ...
        .then((response) => {
          if (response.code == 0) {
             // _this.tableDatas = response.data.records;
            _this.tableDatas3 = JSON.parse(JSON.stringify(response.data.records)); 
            // _this.total = response.data.total;
            _this.total = JSON.parse(JSON.stringify(response.data.total));
            // _this.totalPage = response.data.pages;
            _this.totalPage = JSON.parse(JSON.stringify(response.data.pages));
    ...
    selectentErpriseVSBook() {
      ...
      _this.$axios
        ...
        .then((response) => {
          if (response.code == 0) {
             // _this.tableDatas = response.data.records;
            _this.tableDatas4 = JSON.parse(JSON.stringify(response.data.records)); 
            // _this.total = response.data.total;
            _this.total = JSON.parse(JSON.stringify(response.data.total));
            // _this.totalPage = response.data.pages;
            _this.totalPage = JSON.parse(JSON.stringify(response.data.pages));
