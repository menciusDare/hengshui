            ...
            <template>
                <div>
                    ...
                    <div slot="footer" class="dialog-footer bg-purple-light" style="text-align: center;padding: 10px">
                        <el-button  type="primary" plain @click="closeInfoDialog">关闭</el-button>
                    </div>
                </div>
            </template>
        </el-dialog>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            // forrmotName1(list,codes){
            //     for(var i = 0;i<list.length;i++){
            //             for(var j= 0;j<codes.length;j++){
            //                 if(list[i].code == codes[j]){
...
let harzardousTypeList= sessionStorage.getItem('harzardousTypeList')
                           ...
                           res.data.wfKindList.forEach((item)=>{
...
item.dicTypeExplain=ele.dicTypeExplain
              ...
              let filteredList = res.data.wfKindList.filter(item=>{             
                  if(item.dicTypeExplain!=undefined ){
                      return item
                         ...
                         _this.vfTypeList =  filteredList 
                            ...
                            _this.$refs.vfChild.wfKindList = filteredList;
                ...
                that.$axios.get(url,{
                    ...
                    function (res) {
                        if(res.code == 0){
                            ...
                            that.formInlinefa.fwType=res.data.fwType;
                            that.formInlinefa.vfName=res.data.vfName;
                            that.formInlinefa.storageId=res.data.storageId;
                            that.formInlinefa.wfColor=res.data.wfColor.split(',');
                            that.formInlinefa.fwForm=res.data.fwForm.split(',');
                            that.formInlinefa.wxFeatures=res.data.wxFeatures.split(',');
                            that.formInlinefa.wxSituation=res.data.wxSituation;
                            that.formInlinefa.harmful=res.data.harmful;
                            that.formInlinefa.equipment=res.data.equipment;
                            that.formInlinefa.measures=res.data.measures;
